...
class Generator {
  ...
  public Reader OpenFrame(FramePath frame) 
  {
    try
    {
      InputStream frameStream = frame.createInputStream();
      this.frame = new BufferedReader(new InputStreamReader(frameStream));
  ...
  public PrintWriter OpenGen(String target) {
    /*
    File f = new File(tab.outputDirectory, target);
    try {
      if (f.exists()) {
        File old = new File(f.getPath() + ".old");
        old.delete(); f.renameTo(old);
      }
      gen = new PrintWriter(new BufferedWriter(new FileWriter(f, false)));
    } catch (Exception e) {
      throw new FatalError("Cannot generate file: " + f.getPath());
    }
    */
    
    //return gen = new PrintWriter(new BufferedWriter(new StringWriter()));

    textWriter = new StringWriter();
    return gen = new PrintWriter(new BufferedWriter(textWriter));
  }

  @SuppressWarnings("resource")
  public void GenCopyright() {
    File copyFr = null;
    if (tab.frameDirectory != null) copyFr = new File(tab.frameDirectory, "Copyright.frame");
    if (copyFr == null || !copyFr.exists()) copyFr = new File(tab.sourceDirectory, "Copyright.frame");
    if (copyFr == null || !copyFr.exists()) return;

    try {
      Reader scannerFram = frame;
      frame = new BufferedReader(new FileReader(copyFr));
      CopyFramePart(null);
      frame = scannerFram;
...
public class DFA {
  ...
  private final FramePath scannerPath;
    ...
    for (Target t = a.target; t != null; t = t.next) {
      ...
      if (t.state.endOf != null)
        if (endOf == null || endOf == t.state.endOf)
          ...
          errors.catchSemanticError("Tokens " + endOf.name + " and " + t.state.endOf.name +
  ...
  public String WriteScanner() {
    Generator generator = new Generator(tab);
    fram = generator.OpenFrame(this.scannerPath);
    gen = generator.OpenGen("Scanner.java");
    ...
    if (tab.packageName != null && tab.packageName.length() > 0) {
      gen.print("package ");
      gen.print(tab.packageName);
      gen.println(";");
    }
    generator.CopyFramePart("-->declarations");
    gen.println("\tstatic final int maxT = " + (tab.terminals.size() - 1) + ";");
    gen.println("\tstatic final int noSym = " + tab.noSymbol.n + ";");
    if (ignoreCase)
    ...
    gen.close();
    return generator.textWriter.toString();
  }

  public DFA (Parser parser, FramePath scannerPath) {
    this.parser = parser;
    tab = parser.tab;
