...
public class Tab {
	...
	public Symbol grammarSymbol;              // root nonterminal; filled by ATG
	public Symbol eofSymbol;               // end of file symbol
	public Symbol noSymbol;               // used in case of an error
	public BitSet allSyncSets;         // union of all synchronisation sets
	public Hashtable literals;         // symbols that are used as literals

	public String sourceName;             // name of the atg file (including path)
	public String sourceDirectory;              // directory path of the atg file
	public String packageName;              // package name for generated files
	public String frameDirectory;            // directory containing the frame files
	public String outputDirectory;              // directory for generated files
	...
	void CompFollowSets() {
		...
		grammarSymbol.follow.set(eofSymbol.n);
	...
	public void CompDeletableSymbols() {
		...
		for (int i = 0; i < nonterminals.size(); i++) {
			sym = (Symbol)nonterminals.get(i);
			if (sym.deletable) errors.catchWarning("  " + sym.name + " deletable");
		}
	}
  ...
  public String Escape (String s) {
	...
	public boolean NoCircularProductions() {
		...
		for (int i = 0; i < list.size(); i++) {
			CNode n = (CNode)list.get(i);
			ok = false;
			errors.catchSemanticError("  " + n.left.name + " --> " + n.right.name);
		}
		return ok;
	}
	...
	void ResErr(Node p, String msg) {
		errors.catchWarning(p.line, p.pos.col, msg);
	}
	...
	public boolean NtsComplete() {
		...
		for (int i = 0; i < nonterminals.size(); i++) {
			...
			if (sym.graph == null) {
				complete = false;
				errors.catchSemanticError("  No production for " + sym.name);
			}
		}
		return complete;
	}
	...
	public boolean AllNtReached() {
		...
		visited.set(grammarSymbol.n);
		MarkReachedNts(grammarSymbol.graph);
		for (int i = 0; i < nonterminals.size(); i++) {
			Symbol sym = (Symbol)nonterminals.get(i);
			if (!visited.get(sym.n)) {
				ok = false;
				errors.catchWarning("  " + sym.name + " cannot be reached");
			}
		}
		return ok;
	}
	...
	public boolean AllNtToTerm() {
		...
		for (int i = 0; i < nonterminals.size(); i++) {
			...
			if (!mark.get(sym.n)) {
				ok = false;
				errors.catchSemanticError("  " + sym.name + " cannot be derived to terminals");
			}
		}
		return ok;
	}
